  ...
  return (
    <div className='flex justify-between w-full p-4 mb-4 rounded-lg font-montserrat hover:shadow-inner insert-shadow-sm'>
      <div className='flex gap-4'>  
        <div className="1/8">
          <img className='w-40 min-w-[160px]' src={item.images[0]} alt={`Image of ${item.name}`} />
        </div>
      ...
      <div className='flex items-center'>
        <button onClick={() => removeItemHandler()}>
          <img className='min-w-[24px] min-h-[24px]' width={24} src={trash} alt="remove from cart button" />
        </button>
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            <div className="flex justify-end gap-4">
              ...
              <button onClick={confirmRemoval} className="px-4 py-2 bg-red-500 text-white rounded-md">Remove</button>
            </div>
          </div>
        </div>
      )}
